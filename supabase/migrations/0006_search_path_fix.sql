-- pgcrypto is installed in the "extensions" schema on Supabase; the verify
-- function pinned search_path = public, so digest()/crypt() were never
-- resolvable inside it and every verification failed with an exception.
