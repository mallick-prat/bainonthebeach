...
export const UMBRELLAS: Array<{ x: number; y: number; color: "red" | "yellow" }> = [
  { x: 305, y: 192, color: "red" },
  { x: 361, y: 260, color: "yellow" },
];

export const LOUNGERS: Array<{ x: number; y: number; color: "cyan" | "white" }> = [
...
const WIGGLE = [
  0.04, 0.07, 0.03, 0.08, 0.05, 0.02, 0.06, 0.09, 0.04, 0.03, 0.07, 0.05,
  0.02, 0.08, 0.06, 0.03, 0.09, 0.05, 0.07, 0.02, 0.06, 0.04, 0.08, 0.05,
];
...
export function blockers(): CircleBlocker[] {
  ...
  for (const u of UMBRELLAS) list.push({ x: u.x, y: u.y, r: 10, kind: "umbrella" });
  for (const l of LOUNGERS) list.push({ x: l.x, y: l.y, r: 8, kind: "lounger" });
